...
notes:
Fixed point integers are stored as whole integers.
To store x we use:
round(x << frac_width)

This shifts the bits so we can store a fractional number in an int.
Then we round as if there still is a fractional part we cannot save it using our limited frac_width.

Then when reusing the fixed point we can simply:
x >> frac_width
Getting back the original value we wanted to store.
